...
| 2 | ~~Formspree 表单后端~~ **已完成** | ✅ 账号 `yale@tpematsfactory.com`（已验证），表单 `Wholesale Quote Request`，form ID **`mvkgzrqr`**，已接入全部 99 个页面的询价弹窗并线上实测生效。询盘现在有三条腿：WhatsApp 按钮 + 邮件 + 表单落库（详见 DEPLOY.md 6.7） |
